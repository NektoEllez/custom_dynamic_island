...
final class DynamicIslandProgressIndicatorImplementation: UIView {
    ...
    func add(toContext context: UIWindow) {
        /// Добавляет индикатор прогресса к указанному окну.
        /// Создает и добавляет слои индикатора на заданное окно.
        /// Также устанавливает индикатор как видимый.
        requiresState(equalTo: .ready)
    ...
    func changeContext(to newContext: UIWindow) {
        /// Изменяет контекст (окно), к которому прикреплен индикатор.
                /// Удаляет текущий индикатор и добавляет его к новому контексту.
        requiresState(equalTo: .ready)
        removeIndicator()
    ...
    func showIndeterminateProgressAnimation() {
        // Отображает анимацию индикатора неопределенного прогресса.
        // Сбрасывает индикатор и показывает анимацию, делая его видимым.
        // Анимация создает вращение индикатора.
...
//    }
    ...
    private func mainTailAnimation() -> CABasicAnimation {
        let animation = CABasicAnimation(keyPath: "strokeEnd")
        animation.fromValue = 0
        animation.toValue = 1
        animation.duration = 1 // Уменьшение продолжительности анимации
...
/// Обертка свойства, ограничивающая значение в заданном диапазоне.
@propertyWrapper
